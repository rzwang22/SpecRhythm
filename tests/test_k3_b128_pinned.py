...
ENTRY = Path(__file__).resolve().parents[1] / "scripts/run_k3_b128_pinned.sh"
SHA = "f7bfb43152f5655edbad9888c53c0a81e9d11954"


@pytest.mark.parametrize(
    "configuration,scale", [("unified", "b128"), ("io-only", "b128"), ("unified", "b64")]
)
@pytest.mark.parametrize("failure", [0, 23])
def test_fixed_launcher_passes_exact_commit_and_config(tmp_path, configuration, scale, failure):
    binary = tmp_path / "bin"
    binary.mkdir()
    git = binary / "git"
...
if 'worktree' in args:
 ...
 (root/('scripts/run_k3_'+os.environ['SCALE']+'.sh')).write_text(''' + repr('''#!/bin/bash
[[ "$1" == "$SHA" ]] || exit 81
...
''')
    ...
    env = {**os.environ, "PATH": str(binary) + os.pathsep + os.environ["PATH"],
           ...
           "SR_K3_DRAFT_DISPATCH": "legacy", "SCALE": scale}
    args = [] if configuration == "unified" and scale == "b128" else [configuration, scale]
...
@pytest.mark.parametrize("args", [["other"], ["unified", "b16"]])
