...
ENTRY = Path(__file__).resolve().parents[1] / "scripts/run_k3_b64_diagnostics_pinned.sh"
SHA = "cc42a501b63623de3e046e6388d0dcab0d2f339c"


@pytest.mark.parametrize("configuration", ["io-only", "unified"])
...
if 'worktree' in args:
 ...
 (root/'scripts/run_k3_b64.sh').write_text(''' + repr('''#!/bin/bash
[[ "$1" == "$SHA" ]] || exit 81
...
''')
    ...
    env = {**os.environ, "PATH": str(binary) + os.pathsep + os.environ["PATH"],
           ...
           "SR_K3_REPEAT_CHILD": "stale-inherited-flag"}
    result = subprocess.run(["bash", str(ENTRY), configuration], env=env,
...
def test_invalid_configuration_stops_before_checkout():
    result = subprocess.run(["bash", str(ENTRY), "other"], capture_output=True, text=True,
                            timeout=20)
