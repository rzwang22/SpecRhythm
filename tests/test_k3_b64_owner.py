...
from specrhythm.serving.k3 import B64, MODES, geometry
...
def test_B64_owner_target_no_hidden_B16_limit(target_pool, monkeypatch, mode):
    ...
    ids = list(s.requests)[:64]
    packet.update(k3_configuration=B64, active_limit=64,
    ...
    for i, (rid, r) in enumerate(s.requests.items()):
        ...
        packet['requests'][rid]['cohort'] = 'A' if g['serial_idle_gate'] or i < 32 else 'B'
    ...
    try:
        packet['pp_admission'] = PingPrePostController(mode, B64).select(
        ...
        assert len(forwards) == 2 and {f['B'] for f in forwards} == {64}
        assert owner.machine.backend.physical_batch_ceiling == 64
