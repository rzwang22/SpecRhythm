...
if bash -s -- "$@" <<'SR_K3_TARGET_CHILD'
set -Eeuo pipefail
[[ $# -le 1 ]] || { printf 'Choose one of baseline or lean-target.\n' >&2; exit 2; }
EXPERIMENT="${1:-lean-target}"
case "$EXPERIMENT" in
  baseline|lean-target) ;;
  *) printf 'Choose baseline (full) or lean-target; no dispatch variant is claimed.\n' >&2; exit 2 ;;
esac
FINAL_SHA=4ff1170397db0393caa2d8a43c2089a8e602ddfd
