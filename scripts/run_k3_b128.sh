...
export SR_K3_EXPERIMENT="${SR_K3_EXPERIMENT:-unified}"
...
if [[ -z "${SR_K3_LOCAL_DELIVERY:-}" ]]; then
  ...
  exec "$SR_FIXED_PYTHON" -m specrhythm.serving.k3_local_run --repo "$PWD" --commit "$FINAL_SHA" --k3-configuration k3-b128-v1 --validation-profile "$SR_K3_VALIDATION_PROFILE"
fi
if [[ -n "${SR_K3_EXPERIMENT:-}" && -z "${SR_K3_REPEAT_CHILD:-}" ]]; then
  ...
  exec "$SR_FIXED_PYTHON" -m specrhythm.serving.k3_repeat_run \
    ...
    --configuration "$SR_K3_EXPERIMENT" --k3-configuration k3-b128-v1
fi
# Only the repeat controller may reuse the first four completed capacity probes.
if [[ "${SR_K3_SKIP_CAPACITY:-0}" == 1 ]]; then
  [[ "${SR_K3_REPEAT_CHILD:-}" == 1 && "${SR_K3_REVERSE:-}" == 1 ]]
fi
export SR_K3_OBSERVATION="${SR_K3_OBSERVATION:-deferred-window}"
DISPATCH_ARGS=()
...
TARGET_DISPATCH_ARGS=()
if [[ -n "${SR_K3_TARGET_DISPATCH:-}" ]]; then
  TARGET_DISPATCH_ARGS=(--target-dispatch "$SR_K3_TARGET_DISPATCH")
fi
...
write_once(pathlib.Path(os.environ['SR_PING_DELIVERY'])/'validation-plan.json',
           plan(os.environ['SR_K3_VALIDATION_PROFILE'], 'k3-b128-v1'))
PY_POLICY
STAGE=static_capacity_contract
"$SR_FIXED_PYTHON" -m specrhythm.serving.k3_capacity \
  --output "$SR_PING_DELIVERY/k3-capacity-contract.json" --k3-configuration k3-b128-v1
...
for POINT in "${MODES[@]}"; do
  ...
  bash scripts/run_decode_scan.sh prepare --k3-configuration k3-b128-v1 --validation-profile "$SR_K3_VALIDATION_PROFILE" --s1 "$SR_FIXED_S1" --draft-audit runtime \
    --observation "$SR_K3_OBSERVATION" ${DISPATCH_ARGS[@]+"${DISPATCH_ARGS[@]}"} ${TARGET_DIAGNOSTIC_ARGS[@]+"${TARGET_DIAGNOSTIC_ARGS[@]}"} ${TARGET_DISPATCH_ARGS[@]+"${TARGET_DISPATCH_ARGS[@]}"} --identity-matching bound-prefix --selection-seed 1666 \
...
assert o.get('target_dispatch')==os.environ.get('SR_K3_TARGET_DISPATCH')
...
g=geometry(os.environ['SR_AUDIT_SERVING_MODE'], "k3-b128-v1")
assert matches_geometry(c['execution_geometry'],os.environ['SR_AUDIT_SERVING_MODE'], 'k3-b128-v1')
assert s['k3_configuration']==m['k3_configuration']==c['k3_configuration']=='k3-b128-v1'
assert s['validation_profile']==m['validation_profile']==os.environ['SR_K3_VALIDATION_PROFILE']
assert (m['active_limit'],c['per_cohort_capacity'],c['max_requests_per_target_forward'])==(128,max(g['home_capacities'].values()),g['target_request_ceiling'])
assert c['proposal_budget']==3
PY_CONFIG
  if [[ "${SR_K3_SKIP_CAPACITY:-0}" != 1 ]]; then
    STAGE=capacity
    bash scripts/run_decode_scan.sh capacity --single-point --batch 128 --mode "$POINT"
  fi
done
...
"$SR_FIXED_PYTHON" -m specrhythm.serving.k3_gpu_check \
  --source "$SR_PING_DELIVERY/points/${MODES[0]}" --output "$SR_PING_DELIVERY/joint" --k3-configuration k3-b128-v1
else
...
for POINT in "${MODES[@]}"; do
  export SR_AUDIT_SERVING_MODE="$POINT" SR_FIXED_ROOT="$SR_PING_DELIVERY/points/$POINT"
  STAGE=execution
  bash scripts/run_decode_scan.sh run --single-point --batch 128 --mode "$POINT"
  STAGE=measurement
...
measurement(r,read_json(pathlib.Path(r['artifact'])/'runtime.json'),mode, 'k3-b128-v1',
            validation_profile=os.environ['SR_K3_VALIDATION_PROFILE'])
g=geometry(mode, "k3-b128-v1")
...
PY_MEASUREMENT
  ...
  "$SR_FIXED_PYTHON" -m specrhythm.serving.ping_prepost_evidence \
    --root "$SR_FIXED_ROOT" --commit "$FINAL_SHA" --k3-configuration k3-b128-v1 \
