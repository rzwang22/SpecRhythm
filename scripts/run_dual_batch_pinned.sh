...
TAG="a100-ordinary-dual-$(date -u +%Y%m%dT%H%M%SZ)-$$"
RUN_TREE="${REPO}-ordinary-dual-${FINAL_SHA:0:12}-${TAG}"
git -C "$REPO" worktree add --detach "$RUN_TREE" "$FINAL_SHA"
test -f "$RUN_TREE/scripts/run_dual_batch.sh"
...
unset SR_K3_TARGET_DISPATCH SR_K3_EXPERIMENT SR_K3_TARGET_EXPERIMENT
...
bash "$RUN_TREE/scripts/run_dual_batch.sh" "$FINAL_SHA"
SR_DUAL_CHILD
then
  printf 'Ordinary double-batch comparison finished; return the single archive above.\n'
else
  rc=$?
