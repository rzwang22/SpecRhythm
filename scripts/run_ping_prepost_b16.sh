...
export SR_PING_DELIVERY="$RESULTS/pingpong-prepost3-delivery-${RUN_TAG}"
...
PY_FAILURE
  ...
  "$SR_FIXED_PYTHON" -m specrhythm.serving.ping_prepost_delivery \
    --directory "$SR_PING_DELIVERY" --output "$ARCHIVE" --first-code "$first_rc" --stage "$STAGE" \
...
from specrhythm.serving.audit_layer_report import write
p=pathlib.Path(os.environ['SR_PING_DELIVERY']); v=comparison(p)
