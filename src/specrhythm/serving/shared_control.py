"""Shared ordinary K3 control commands; no second engine or KV owner.
...
def enabled():
    return os.environ.get("SR_K3_TARGET_DISPATCH") in ("dual-batch", "shared-command")


def validate_mode(mode):
    if os.environ.get("SR_K3_TARGET_DISPATCH") == "shared-command":
        require(mode in ("pingpong-k3", "serial-k3"),
                "shared-command requires ordinary Serial or PingPong K3")
    else:
