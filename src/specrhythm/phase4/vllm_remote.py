...
class RemoteDraftProposer:
        ...
        *,
        ...
        if self.resident_mode and not self.resident_setup_complete:
            if target_materialized_token_counts is None:
                raise RuntimeError(
                    "resident Serial requires materialized-token worker evidence"
                )
            setup = (
    ...
    def _rank_zero_observe_resident_setup(
        ...
        for row in rows:
            self._log_resident_setup_row(row)
            ...
            if stable_id != row.stable_request_id:
                raise RuntimeError("resident Serial classifier/identity binding differs")
    ...
    def _log_resident_setup_row(self, row: ResidentSetupRow) -> None:
        assert self.decode_ready_timing_log is not None
        self.decode_ready_timing_log.append(
            {
